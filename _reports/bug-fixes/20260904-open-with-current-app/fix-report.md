...
- 处理状态：已修复
- 发布状态：最终修复提交 `d28dbc1d0f74db4b704c276f6317ae06fb19dc24` 已推送至 `origin/main`；本机 `/Applications/WTypora.app` 已安装并验证
...
5. 编辑器只在真实用户交互的短暂事件窗口内接受视觉编辑器产生的 Markdown 更新；程序化查找替换直接同步序列化结果，避免延迟的初始化规范化借用旧交互状态触发自动保存。
...
| 初始化点击竞态 | 冷启动样本，1 秒后点击正文，再等待 35 秒并 `cmp` | 通过；内容和 mtime 均不变 |
...
| 本机安装产物 | `/Applications/WTypora.app` 可执行文件摘要与签名校验 | 通过；SHA-256 为 `48e5d7d1a2eaa89e5768b92238bdd35717be3f7df3661489242b7449569be7f3` |
...
补充说明：最终核对曾捕获“初始化完成前点击正文”会延迟规范化空行的竞态；文档只出现空行与末尾换行差异，已立即退出应用并从上述逐字节备份恢复，最终再次确认正文、大小和修改时间与备份完全一致。
