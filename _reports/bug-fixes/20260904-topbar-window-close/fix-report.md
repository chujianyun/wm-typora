...
- 自动重装验证：后续重装时未发现残留运行进程；同一已校验 release 经临时安装位复制、签名与哈希复核后替换成功，替换前版本备份至 `/Users/wuming/.Trash/WTypora-reinstalled-20260904-1230.app`。首次启动 PID `71750`；后续核验时进程已退出，再次启动 PID `72132` 并连续存活 15 秒。
